...
1. **Google Flights** — flight matrix + Google's own price-insight banding. Primary flight signal.
2. **Riu.com packages** (packagesus.riu.com) — flight+hotel bundles for both Riu properties.
3. **CheapCaribbean** — Antillas listing (has resort-credit promos; note promo value separately).
4. **Expedia** — package (flight+hotel) and hotel-only for all three resorts.
5. **Kayak** — cross-check hotel-only nightly rates.
